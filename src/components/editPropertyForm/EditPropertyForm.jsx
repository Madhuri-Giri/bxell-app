...
import { submitSellPropertyForm, editPropertyDetail } from "../../API/apiServices";
  ...
  if (!propertyData) {
    ...
    const [formData, setFormData] = useState({
     ...
     const [imagePreview, setImagePreview] = useState([]);
      ...
      useEffect(() => {
        ...
        : [propertyData.file_name]; // Wrap in an array if it's not in JSON format already
    
      setImagePreview(parsedImages);
    
        if (propertyData) {
          console.log(propertyData); // Check the data structure here
          setFormData({
            ...
            bathroom: businessData.bathroom || "",
   ...
   const UpdateForm = async (e) => {
      ...
      try {
        ...
        const response = await editPropertyDetail(formDataObject);
  
        // Handle response from the API
        if (response) {
          toast.success("property form updated successfully!");  // Show success toast
